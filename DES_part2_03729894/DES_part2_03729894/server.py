...
class Server(object):
        ...
        if self.server_busy:
            ...
            if pkt_arrived is None:
                ...
                if self.sim.sys_state.buffer.is_empty():
                    ...
                    self.packet_in_service = self.sim.sys_state.buffer.remove() # could return None but is not an issue as we checked is_empty first
            else:
    ...
    def complete_service(self):
        ...
        #######################################
        # TODO Task 2.2.4: Your code goes here
        self.packet_in_service.complete_service(self.sim.sys_state.now)
        #######################################

        #######################################
        # TODO Task 2.4.3: Your code goes here
        self.sim.statistics_collection.count_packet(self.packet_in_service)
        self.packet_in_service = None
        #######################################
