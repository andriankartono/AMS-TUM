...
class PacketArrival(SimEvent):
    ...
    def process(self):
        ...
        # create a new packet at the time of arrival
        new_packet = self.user.generate_packet() # generate the actual packet
        self.user.generate_packet_arrival() # generate the event
        
        # Try to bypass the buffer. If not possible -> add to buffer
        if not self.sim.sys_state.start_serving_packet(new_packet):
            self.sim.sys_state.add_packet_to_buffer(new_packet)
