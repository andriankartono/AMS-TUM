...
class SysState(object):
    ...
    def create_servers(self, sim: Simulation, num_servers: int, service_rates: list):
        ...
        for i in range(num_servers):
            holder_list.append(Server(sim, service_rates[i]))
        return holder_list
        #######################################
    ...
    def create_users(self, sim: Simulation, num_users: int, arrival_rates: list):
        ...
        for i in range(num_users):
            holder_list.append(User(sim, arrival_rates[i]))
        return holder_list
        #######################################
    ...
    def add_packet_to_buffer(self, packet:Packet):
        ...
        #######################################
        # TODO Task 2.2.2: Rewrite code from above, your code goes here
        if self.buffer.get_queue_length()<self.sim.sim_param.S:
    ...
    def start_serving_packet(self, pkt_arrived:Packet):
        ...
        for server in self.servers:
            if not server.server_busy:
                # start serving packet
                server.start_service(pkt_arrived)
                return True
