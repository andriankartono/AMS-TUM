...
class Simulation(object):
    ...
    def do_simulation(self):
        ...
        while not self.sys_state.stop:
            ...
            # update the time to the new event time
            self.sys_state.now = event.timestamp

            # process the event-> will terminate by itself when it finds simulation termination
            # the termination will happen because self.sys_state.stop is updated
            event.process()
