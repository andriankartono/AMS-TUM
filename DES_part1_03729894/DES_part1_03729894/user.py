...
class User(object):
    ...
    def generate_packet_arrival(self):
        ...
        self.last_iat = random.uniform(1, 2*(1/self.arrival_rate))
        # create new packetarrival object
        self.new_packetarrival_event = PacketArrival(self.sim, self.sim.sys_state.now + self.last_iat, self)
        self.sim.event_chain.insert(self.new_packetarrival_event)
        #######################################
