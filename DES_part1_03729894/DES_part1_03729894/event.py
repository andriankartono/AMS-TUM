...
class EventChain(object):
    ...
    def insert(self, e: SimEvent):
        ...
        # Smallest element of heapq is always located at the root.
        # Heapq does not mean that all elements are sorted in order. For more explaination see:
...
class SimEvent(object):
    ...
    def process(self):
        ...
        raise NotImplementedError(
            'Please Implement method "process" in subclass of SimEvent'
        )
...
class PacketArrival(SimEvent):
    ...
    def process(self):
        ...
        # insert new packet arrival
        self.user.generate_packet_arrival()
        
         # check if we can bypass the queue
        if self.sim.sys_state.start_serving_packet():
            return
        else:
...
class ServiceCompletion(SimEvent):
    ...
    def process(self):
        ...
        # make the server non busy
        self.server.complete_service()

        # try to process the next packet from the buffer
        self.server.start_service(False)
        #######################################
