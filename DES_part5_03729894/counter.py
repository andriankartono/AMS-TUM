...
class TimeIndependentCounter(Counter):
    ...
    def report_confidence_interval(self, alpha: float = 0.05, print_report: bool = True):
        ...
        half_width = math.sqrt(var/n) * t

        if print_report:
            print(f'Counter {self.name}; confidence interval: [{mean-half_width} ; {mean+half_width}]; n: {n}; half_width: {half_width}; mean: {mean}' )
    ...
    def is_in_confidence_interval(self, x: float, alpha: float = 0.05):
        ...
        return (mean-half_width) <= x and (mean+half_width) >=x
        #######################################
    ...
    def report_bootstrap_confidence_interval(self, alpha: float = 0.05, resample_size: int = 5000,
        ...
        # Do the resampling for the mean
        for _ in range(resample_size):
            samples = numpy.random.choice(self.values, n, replace=True)
            means.append(numpy.mean(samples))

        lower_quantile_index = int(alpha/2 * resample_size)
        upper_quantile_index = int(resample_size * (1-alpha/2))

        # bootstrap difference
        devs = numpy.subtract(means, mean)
        lower_half_width = numpy.sort(devs)[upper_quantile_index]
        upper_half_width = numpy.sort(devs)[lower_quantile_index]
        lower_limit = mean - lower_half_width
        upper_limit = mean - upper_half_width

        if print_report:
            print(f'Counter {self.name}; confidence interval: [{lower_limit} ; {upper_limit}]; n: {n}; half_width: {lower_half_width} and {upper_half_width}; mean: {mean}' )
